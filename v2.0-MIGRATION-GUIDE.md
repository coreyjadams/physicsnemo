...
v2.3, and the `PHYSICSNEMO_ENABLE_COMPAT` environment variable no longer has any
effect. The tables below list every alias it provided; update imports accordingly.

### Moved Modules

Any name previously imported from the module on the left is now imported from the
module on the right.

| Before (pre-v2.0) | After |
|---|---|
