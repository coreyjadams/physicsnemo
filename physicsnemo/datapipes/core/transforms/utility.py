...
@register()
class ZeroLike(ConstantField):
    r"""
    Create a zero tensor with shape based on a reference tensor.

    This is a convenience alias for ConstantField with fill_value=0.0.

    .. deprecated::
        Use :class:`ConstantField` with ``fill_value=0.0`` instead.
        This class is maintained for backwards compatibility.

    Creates a tensor of zeros where the first dimension matches the reference
    tensor and the last dimension is configurable. Useful for creating placeholder
    tensors like zero SDF values for surface points.
    ...
    def __init__(
        ...
        *,
        ...
        """
        Initialize the zero tensor creation transform.

        Args:
            reference_key: Key for the tensor to use as shape reference.
            output_key: Key to store the zero tensor.
            output_dim: Feature dimension for output tensor. Creates tensor with
                       shape (N, output_dim) where N is the first dimension of
                       the reference tensor. If None, defaults to 1.
        """
        super().__init__(
